...
#define LOGGER(__LEVEL__, ...) log_msg(__LEVEL__, __FILENAME__, __LINE__, __VA_ARGS__)
void log_msg(int loglevel, char* file, int line, char *fmt_str, ...) {
  va_list ap;
  char buf[2048];
  ...
  if (foreground) {
		if (!error_only || (error_only && loglevel <= LOG_ERR)) {
    	fprintf(stderr, "[%d:LEVEL(%d):%s:%s(%d)] %s\n", getpid(), loglevel, PACKAGE, file, line, buf);
		}
  } else {
...
static int create_recv_sock() {
  ...
  if (sd < 0) {
    LOGGER(LOG_ERR, "recv socket(): %s", strerror(errno));
    return sd;
  }
  ...
  if ((r = setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) < 0) {
    LOGGER(LOG_ERR, "recv setsockopt(SO_REUSEADDR): %s", strerror(errno));
    return r;
  }
  ...
  if ((r = bind(sd, (struct sockaddr *)&serveraddr, sizeof(serveraddr))) < 0) {
    LOGGER(LOG_ERR, "recv bind(): %s", strerror(errno));
  }

  // enable loopback in case someone else needs the data
  if ((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_LOOP, &on, sizeof(on))) < 0) {
    LOGGER(LOG_ERR, "recv setsockopt(IP_MULTICAST_LOOP): %s", strerror(errno));
    return r;
  }

#ifdef IP_PKTINFO
  if ((r = setsockopt(sd, SOL_IP, IP_PKTINFO, &on, sizeof(on))) < 0) {
    LOGGER(LOG_ERR, "recv setsockopt(IP_PKTINFO): %s", strerror(errno));
    return r;
  }
#endif
...
static int create_send_sock(int recv_sockfd, const char *ifname, struct if_sock *sockdata) {
  int sd = socket(AF_INET, SOCK_DGRAM, 0);
  if (sd < 0) {
    LOGGER(LOG_ERR, "send socket(): %s", strerror(errno));
    return sd;
  }
  ...
  if ((r = setsockopt(sd, SOL_SOCKET, SO_BINDTODEVICE, &ifr, sizeof(struct ifreq))) < 0) {
    LOGGER(LOG_ERR, "send setsockopt(SO_BINDTODEVICE): %s", strerror(errno));
    return r;
  }
#endif
  ...
  int on = 1;
  if ((r = setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) < 0) {
    LOGGER(LOG_ERR, "send setsockopt(SO_REUSEADDR): %s", strerror(errno));
    return r;
  }
  ...
  if ((r = bind(sd, (struct sockaddr *)&serveraddr, sizeof(serveraddr))) < 0) {
    LOGGER(LOG_ERR, "send bind(): %s", strerror(errno));
  }

#if __FreeBSD__
  if((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_IF, &serveraddr.sin_addr, sizeof(serveraddr.sin_addr))) < 0) {
    LOGGER(LOG_ERR, "send ip_multicast_if(): errno %d: %s", errno, strerror(errno));
  }
#endif
  ...
  if ((r = setsockopt(recv_sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq))) < 0) {
    LOGGER(LOG_ERR, "recv setsockopt(IP_ADD_MEMBERSHIP): %s", strerror(errno));
    return r;
  }

  // enable loopback in case someone else needs the data
  if ((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_LOOP, &on, sizeof(on))) < 0) {
    LOGGER(LOG_ERR, "send setsockopt(IP_MULTICAST_LOOP): %s", strerror(errno));
    return r;
  }
  ...
  if ((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl))) < 0) {
    LOGGER(LOG_ERR, "send setsockopt(IP_MULTICAST_TTL): %s", strerror(errno));
    return r;
  }
  ...
  LOGGER(LOG_ERR, "dev %s addr %s mask %s net %s", ifr.ifr_name, addr_str, mask_str, net_str);
  free(addr_str);
  free(mask_str);
  free(net_str);
...
static void daemonize() {
  ...
  umask(0027);
  if (chdir("/") < 0) {
    LOGGER(LOG_ERR, "unable to change to root directory");
    exit(1);
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (open("/dev/null", O_RDWR) != i) {
      LOGGER(LOG_ERR, "unable to open /dev/null for fd %d", i);
      exit(1);
    }
  }
  ...
  if (running_pid != -1) {
    LOGGER(LOG_ERR, "already running as pid %d", running_pid);
    exit(1);
  } else if (! write_pidfile()) {
    LOGGER(LOG_ERR, "unable to write pid file %s", pid_file);
    exit(1);
  }
}
...
static void switch_user() {
  ...
  if (setgid(user->pw_gid) != 0) {
    LOGGER(LOG_ERR, "Failed to switch to group %d - %s", user->pw_gid, strerror(errno));
    exit(2);
  } else if (setuid(user->pw_uid) != 0) {
    LOGGER(LOG_ERR, "Failed to switch to user %s (%d) - %s", user->pw_name, user->pw_uid, strerror(errno));
    exit(2);
  }
}
...
static void show_help(const char *progname) {
  ...
  fprintf(stderr, "\n"
          ...
          "  -e  print error message only in foreground\n"
...
static int parse_opts(int argc, char *argv[]) {
  ...
  struct subnet *ss;
  char *msg;
  while ((c = getopt(argc, argv, "hfep:b:w:u:")) != -1) {
    switch (c) {
      case 'h': help = 1; break;
			case 'e': error_only = 1; break;
      case 'f': foreground = 1; break;
      case 'p':
        if (optarg[0] != '/')
          LOGGER(LOG_ERR, "pid file path must be absolute");
        else
          pid_file = optarg;
        break;
      ...
      case 'b':
        if (num_blacklisted_subnets >= MAX_SUBNETS) {
          LOGGER(LOG_ERR, "too many blacklisted subnets (maximum is %d)", MAX_SUBNETS);
          exit(2);
        }

        if (num_whitelisted_subnets != 0) {
          LOGGER(LOG_ERR, "simultaneous whitelisting and blacklisting does not make sense");
          exit(2);
        }
        ...
        switch (res) {
          case -1:
            LOGGER(LOG_ERR, "invalid blacklist argument");
            exit(2);
          case -2:
            LOGGER(LOG_ERR, "could not parse netmask");
            exit(2);
          case -3:
            LOGGER(LOG_ERR, "invalid netmask");
            exit(2);
        }
        ...
        memset(msg, 0, 128);
        tostring(ss, msg, 128);
        LOGGER(LOG_INFO, "blacklist %s", msg);
        free(msg);
        break;
      case 'w':
        if (num_whitelisted_subnets >= MAX_SUBNETS) {
          LOGGER(LOG_ERR, "too many whitelisted subnets (maximum is %d)", MAX_SUBNETS);
          exit(2);
        }
        ...
        switch (res) {
          case -1:
            LOGGER(LOG_ERR, "invalid whitelist argument");
            exit(2);
          case -2:
            LOGGER(LOG_ERR, "could not parse netmask");
            exit(2);
        ...
        memset(msg, 0, 128);
        tostring(ss, msg, 128);
        LOGGER(LOG_INFO, "whitelist %s", msg);
        free(msg);
        break;
      case '?':
      case ':':
      ...
      case 'u': {
        if ((user = getpwnam(optarg)) == NULL) {
          LOGGER(LOG_ERR, "No such user '%s'", optarg);
          exit(2);
        }
        break;
      }
...
int main(int argc, char *argv[]) {
  ...
  if ((argc - optind) <= 1) {
    show_help(argv[0]);
    LOGGER(LOG_ERR, "error: at least 2 interfaces must be specified");
    exit(2);
  }
  ...
  if (server_sockfd < 0) {
    LOGGER(LOG_ERR, "unable to create server socket");
    r = 1;
    goto end_main;
  }
  ...
  for (i = optind; i < argc; i++) {
    if (num_socks >= MAX_SOCKS) {
      LOGGER(LOG_ERR, "too many sockets (maximum is %d)", MAX_SOCKS);
      exit(2);
    }

    int sockfd = create_send_sock(server_sockfd, argv[i], &socks[num_socks]);
    if (sockfd < 0) {
      LOGGER(LOG_ERR, "unable to create socket for interface %s", argv[i]);
      r = 1;
      goto end_main;
    }
    num_socks++;
  ...
  if (pkt_data == NULL) {
    LOGGER(LOG_ERR, "cannot malloc() packet buffer: %s", strerror(errno));
    r = 1;
    goto end_main;
  }
  ...
  while (! shutdown_flag) {
    ...
    if (FD_ISSET(server_sockfd, &sockfd_set)) {
      ...
      if (recvsize < 0) {
        LOGGER(LOG_ERR, "recv(): %s", strerror(errno));
      }
      ...
      if (num_whitelisted_subnets != 0) {
        ...
        if (!whitelisted_packet) {
					LOGGER(LOG_INFO, "skipping packet from=%s size=%zd", inet_ntoa(fromaddr.sin_addr), recvsize);
          continue;
        }
      } else {
      ...
      LOGGER(LOG_INFO, "data from=%s size=%zd", inet_ntoa(fromaddr.sin_addr), recvsize);
      ...
      for (j = 0; j < num_socks; j++) {
        ...
        LOGGER(LOG_INFO, "repeating data to %s", socks[j].ifname);

        // repeat data
        ssize_t sentsize = send_packet(socks[j].sockfd, pkt_data, (size_t) recvsize);
        if (sentsize != recvsize) {
          if (sentsize < 0)
            LOGGER(LOG_ERR, "send(): %s", strerror(errno));
          else
            LOGGER(LOG_ERR, "send_packet size differs: sent=%zd actual=%zd",
              recvsize, sentsize);
        }
      }
    }
